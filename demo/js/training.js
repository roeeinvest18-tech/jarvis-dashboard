...
function markWorkoutDone(date) {
  ...
  try {
    ...
    if (typeof renderDailyToday === 'function' && date === todayIso()) renderDailyToday();
  } catch (e) { /* the training log must never fail because of this */ }
}

// --- hints ---------------------------------------------------------------
const TRAINING_HINTS_DISMISSED_KEY = 'jarvis:training:hintsDismissed';

function renderTrainingHintsHtml(sessions) {
  if (typeof trainingVisibleHints !== 'function') return '';
  const labels = Object.fromEntries(TRAINING_DRILLS.map(d => [d.id, d.label]));
  const dismissed = loadTrainingJson(TRAINING_HINTS_DISMISSED_KEY, []);
  const hints = trainingVisibleHints(sessions, labels, todayIso(), dismissed);
  if (!hints.length) return '';
  return `<ul class="training-hints">${hints.map(h => `
...
function wireTrainingHints(payload) {
  document.querySelectorAll('[data-hint-dismiss]').forEach(btn => {
    btn.addEventListener('click', () => {
      const dismissed = loadTrainingJson(TRAINING_HINTS_DISMISSED_KEY, []);
      dismissed.push(btn.dataset.hintDismiss);
      saveTrainingJson(TRAINING_HINTS_DISMISSED_KEY, dismissed);
      renderTrainingZone(payload);
    });
  });
}
...
function renderTrainingZone(payload) {
  ...
  mount.innerHTML = `
    ...
    ${renderTrainingHintsHtml(sessions)}
