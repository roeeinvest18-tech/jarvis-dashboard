...
const DASHBOARD = {
  ...
  async fetchAll() {
    // Tasks, Email, Calendar, the CCI watchlist and the published training
    // seed are no longer published (Phase 7). Training still renders from
    // this device and the sync server; the rest are gone from the app.
    const keys = ['scan', 'breakoutAlerts', 'training', 'build'];
    this.missing.clear();
