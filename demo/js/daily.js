...
const DAILY = {
  ...
  // --- experiments --------------------------------------------------------
  //
  // One behavioural experiment at a time is the intent: a hypothesis, the
  // metric it should move, the baseline it started from, the period, and
  // afterwards what actually happened. The store, sync and tests for this
  // shape already existed; this is the write path the UI uses.
  saveExperiment(patch) {
    const all = this.experiments();
    const now = new Date().toISOString();
    let exp = patch.id ? all.find(e => e.id === patch.id) : null;
    if (exp) {
      Object.assign(exp, patch, { updated_at: now });
    } else {
      exp = {
        id: this.newId('experiment'),
        name: '', hypothesis: '', metric: '', baseline: '',
        started_on: dailyTodayIso(), ends_on: '', result: '', status: 'running',
        created_at: now,
        ...patch,
        updated_at: now,
      };
    ...
    this.saveJson(DAILY_KEYS.experiments, all);
    return exp;
  },

  deleteExperiment(id) {
    this.saveJson(DAILY_KEYS.experiments, this.experiments().filter(e => e.id !== id));
...
// Days on which trades were taken, from the private account record, against
// the same metrics as everything else. Counts only -- the record carries no
// money, and this never runs on a device without the owner's sync token.
function dailyTradeDayObservations(records, tradesByDate) {
  const byDate = tradesByDate || {};
  if (!Object.keys(byDate).length) return [];
  const traded = r => (byDate[r.date] || 0) > 0;
  const out = [];
  for (const metric of ['focus', 'energy', 'sleep_quality']) {
    const cmp = dailyGroupCompare(records, traded, metric);
    if (!cmp.sufficient || cmp.delta === null || Math.abs(cmp.delta) < 0.3) continue;
    const label = { focus: 'Focus', energy: 'Energy', sleep_quality: 'Sleep quality' }[metric];
    out.push({
      text: `${label} averaged ${cmp.a.mean} on days you took a trade (${cmp.a.n} days), `
        + `compared with ${cmp.b.mean} on days you did not (${cmp.b.n} days). `
        + 'An observed association, not a cause.',
      kind: 'association',
      evidence: { metric, a: cmp.a, b: cmp.b },
    });
  }
  // And the reverse reading: how much screen-checking happened on trade days.
  const tvCmp = dailyGroupCompare(records, traded, 'tradingview_opens');
  if (tvCmp.sufficient && tvCmp.delta !== null && Math.abs(tvCmp.delta) >= 1) {
    out.push({
      text: `TradingView was opened ${tvCmp.a.mean} times on average on days you took a trade `
        + `(${tvCmp.a.n} days), compared with ${tvCmp.b.mean} on days you did not `
        + `(${tvCmp.b.n} days). An observed association, not a cause.`,
      kind: 'association',
      evidence: { metric: 'tradingview_opens', a: tvCmp.a, b: tvCmp.b },
    });
  }
  return out;
}
...
function dailyWeeklyReview(endIso) {
  ...
  return {
    ...
    // Training comes from the day records' own Workout habit, which the
    // training log ticks (one source of truth, see training.js).
    trainingSessions: thisWeek.filter(r => r.workout_completed).length,
    previousTrainingSessions: prevWeek.filter(r => r.workout_completed).length,
  };
}
